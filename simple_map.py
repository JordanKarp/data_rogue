...
FLOOR_GRAPHICS: NDArray[np.int32] = np.array(
    [ord(ch) for ch in "    ,.'`"], dtype=np.int32
)
...
def main() -> None:  # noqa: C901, PLR0912
    ...
    world: NDArray[Any] = FLOOR_GRAPHICS[
        np.random.randint(FLOOR_GRAPHICS.size, size=(MAP_HEIGHT, MAP_WIDTH))
    ]  # noqa: NPY002
    ...
    while True:
        ...
        camera_ij = tcod.camera.get_camera(
            console.rgb.shape, (player.y + offset_xy[1], player.x + offset_xy[0])
        )
        if clamp:
            camera_ij = tcod.camera.clamp_camera(
                console.rgb.shape, world.shape, camera_ij, (0.5, 0.5)
            )
        ...
        if cursor_screen_xy:
            cursor_world_xy = (
                cursor_screen_xy[0] + camera_ij[1],
                cursor_screen_xy[1] + camera_ij[0],
            )
        ...
        if (
            cursor_screen_xy
            and 0 <= cursor_screen_xy[0] < console.width
            and 0 <= cursor_screen_xy[1] < console.height
        ):
            console.rgb[["fg", "bg"]][cursor_screen_xy[1], cursor_screen_xy[0]] = (
                0,
                0,
                0,
            ), (255, 255, 255)
        ...
        for event in tcod.event.wait():
            ...
            if isinstance(event, tcod.event.KeyDown):
                ...
                if event.state & tcod.event.BUTTON_RMASK:  # Move offset by tile motion.
                    offset_xy = (
                        offset_xy[0] - event.tile_motion.x,
                        offset_xy[1] - event.tile_motion.y,
                    )
                cursor_screen_xy = (
                    event.tile.x,
                    event.tile.y,
                )  # Save screen cursor position.
            elif isinstance(event, tcod.event.WindowEvent) and event.type == "WINDOWLEAVE":  # type: ignore[comparison-overlap]
                cursor_screen_xy = cursor_world_xy = (
                    None  # Reset cursor when the mouse leaves the window.
                )

...
class PopupMessage(BaseEventHandler):
    ...
    def on_render(self, console: tcod.Console, fade: int = 8, align="center") -> None:
        ...
        if align == "center":
            align = libtcodpy.CENTER
        elif align == "left":
            align = libtcodpy.LEFT
        elif align == "right":
            align = libtcodpy.RIGHT
...
class DialogHandler(AskUserEventHandler):
    ...
    def on_render(self, console):
        ...
        message = self.npc.dialog.display_log()

        message += "\n\n\n\n\n\n\n"
        for i, choice in enumerate(self.npc.dialog.get_choices()):
            if i == self.cursor:
                message += change_text_color(f"> {choice['text']}\n", color.blue)
            else:
                message += f"{choice['text']}\n"
        ...
        console.draw_frame(
            ...
            title=f"Chat with {self.npc.name or 'NPC'}",
            clear=True,
            bg=color.black,
            fg=color.white,
        )
        ...
        # self.engine.event_handler = PopupMessage(
        #     MainGameEventHandler(self.engine), node_text
        # ).on_render(console, 2, align="left")
    ...
    def move_cursor(self, event, key):
        ...
        self.cursor += dy
        self.cursor %= len(self.npc.dialog.get_choices())
    ...
    def ev_keydown(self, event):
        ...
        if key in (tcod.event.KeySym.ESCAPE, tcod.event.KeySym.q):
            return MainGameEventHandler(self.engine)

        if key in keys.UP_DOWN_DIRECTIONS:
            return self.move_cursor(event, key)
        elif key in keys.CONFIRM_KEYS:
            action = self.npc.dialog.choose(self.cursor)
            if action:
                # TODO Shop
                print("open shop")
            if self.npc.dialog.current_node is None:
        ...
        #     action = self.npc.dialog.select_choice(index)
        #     if action == "open_shop":
        #         # TODO Shop
